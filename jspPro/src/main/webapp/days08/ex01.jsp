...
<div>
  <xmp class="code">
      1. [Ajax] 에이작스, 아작스
      [A]synchronus   [J]avacript  [A]nd  [X]ML
      비동기적인      자바스크립트   +    XML
      ...
      6. js ajax 처리 순서(과정)
        1) 웹 페이지에서 이벤트 발생
        2) XMLHttpRequest 객체 생성( 나 대신 일할 객체 생성 )
        3) XMLHttpRequest 객체 설정
           ㄱ. 요청 설정
               ...
               A) get 방식
               XMLHttpReqeust객체.open("GET", "/test.jsp?id=admin", true);
               XMLHttpReqeust객체.send();
               
               B) post 방식
               XMLHttpReqeust객체.open("POST","/test.jsp", true);
               XMLHttpReqeust객체.sen("id=admin");
