...
String uri = request.getRequestURI();  // list.ss  write.ss  update.ss ...등등
		...
		try {
			...
			ProductDAO dao = ProductDAO.getInstance();  // 싱글톤
			// ㄴ
			saveDirectory =  request.getRealPath("/product/upload");
			System.out.println(saveDirectory);
			
			if (uri.endsWith("list.ss")) { // 자료실 목록 보기				
				ArrayList<ProductDTO> list = dao.selectFileList(conn);
				request.setAttribute("list", list);
				
				String path = "/product/ex01_list.jsp";
				RequestDispatcher dispatcher = request.getRequestDispatcher(path);
				dispatcher.forward(request, response);
				
			} else if (uri.endsWith("write.ss")) { // 글쓰기 + write.jsp
				String path = "/product/ex01_write.jsp";
				RequestDispatcher dispatcher = request.getRequestDispatcher(path);
				dispatcher.forward(request, response);
				...
				int num = Integer.parseInt(request.getParameter("num"));
				ProductDTO dto = dao.selectOne(conn, num);
				request.setAttribute("dto", dto);
				
				String path = "/product/ex01_update.jsp";
				RequestDispatcher dispatcher = request.getRequestDispatcher(path);
				dispatcher.forward(request, response);				
	            ...
	            // 수정할 때 첨부파일 있다면 벌써 upload 폴더 저장 완료!!!
	             ...
	             if (attachFile != null) {  // 첨부파일이 있을 경우에만
	               ...
	               String deleteFilePath = 
	            		   String.format("%s\\%s", saveDirectory, delete_filesystemname);
	               File deleteFile = new File(deleteFilePath);
	               if( deleteFile.exists() ) deleteFile.delete();	 
	             
	             } else if (!delete_filesystemname.equals("")){
	            	 	ProductDTO dto2 = dao.selectOne(conn, num);
