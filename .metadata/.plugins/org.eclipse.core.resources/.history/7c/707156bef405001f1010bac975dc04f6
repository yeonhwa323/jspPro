...
<div>
  <xmp class="code">
    emp 테이블의 사원정보를 
    jquery ajax 요청으로 json 형태의 결과물을
    받아서 웹페이지에 출력.    
  ...
  $(function(){
	  $("#btn1").on("click", function(){
		  $.ajax({
			 ...
			 url: "ex09_empjson_lib.jsp" // http:json.org
		 	, dataType: "json"
		 	, type: "GET"
		 	, cache: false
		 	, success : function(data, textStatus, jqXHR){
		 		...
		 		$(data.emp).each(function ( index, element ) {
		 			$("#cmbDept")
		 			 .append(`<option value=\${element.empno}>\${element.ename}</option>`);
		 			$("#demo")
		 			 .append(`<li>\${element.empno} : \${element.ename}</li>`);
		 		});		 		
